...
from Box2D.b2 import (edgeShape, circleShape, fixtureDef,
                      polygonShape, revoluteJointDef, contactListener, shape)
...
# Six points starting from -40 for IR sensors
    ...
    def __init__(self, world, init_angle, init_x, init_y):
        ...
        self.hull = self.world.CreateDynamicBody(
            ...
            fixtures=[
                ...
                fixtureDef(shape=circleShape(pos=(IR_ARRAY[0][0] * SIZE, # Start of IR sensor array code
                    IR_ARRAY[0][1] * SIZE), radius=1 * SIZE), density=0, isSensor=True, userData=0),
                fixtureDef(shape=circleShape(pos=(IR_ARRAY[1][0] * SIZE,
                    IR_ARRAY[1][1] * SIZE), radius=1 * SIZE), density=0, isSensor=True, userData=1),
                fixtureDef(shape=circleShape(pos=(IR_ARRAY[2][0] * SIZE,
                    IR_ARRAY[2][1] * SIZE), radius=1 * SIZE), density=0, isSensor=True, userData=2),
                fixtureDef(shape=circleShape(pos=(IR_ARRAY[3][0] * SIZE,
                    IR_ARRAY[3][1] * SIZE), radius=1 * SIZE), density=0, isSensor=True, userData=3),
                fixtureDef(shape=circleShape(pos=(IR_ARRAY[4][0] * SIZE,
                    IR_ARRAY[4][1] * SIZE), radius=1 * SIZE), density=0, isSensor=True, userData=4),
                fixtureDef(shape=circleShape(pos=(IR_ARRAY[5][0] * SIZE,
                    IR_ARRAY[5][1] * SIZE), radius=1 * SIZE), density=0, isSensor=True, userData=5)
            ]
        )
    ...
    def draw(self, viewer, draw_particles=True):
        ...
        for obj in self.drawlist:
            for f in obj.fixtures:
                if f.shape.__class__.__name__ == "b2CircleShape":
                    continue
